...
if [ ! -f .done ]; then
  "$WROOT/src/gcc-$GCC/configure" --target=avr --prefix="$WPREFIX" \
      ...
      MAKEINFO=true
  # MAKEINFO=true, not =missing, and repeated on make/install exactly as the
  # binutils stage above does it -- that stage has never failed this way.
  #
  # `missing` is autotools' wrapper that RUNS the real tool when it is present
  # and only no-ops when it is absent. texinfo IS present here: base-devel pulls
  # texinfo 7.2-3, so `missing makeinfo` delegated to a real makeinfo and the
  # build died on documentation:
  #
  #   /c/a/src/gcc-14.2.0/missing makeinfo ... -o doc/cpp.info .../cpp.texi
  #   cpp.texi:9: @include: could not find gcc-common.texi
  #   make[2]: *** [Makefile:3588: doc/cpp.info] Error 1
  #
  # gcc-common.texi is generated by the doc machinery configure had already
  # switched off ("*** Makeinfo is missing. Info documentation will not be
  # built."), while BUILD_INFO stayed `info` so the rule fired anyway. Docs half
  # on and half off. `true` is a no-op that always succeeds, so makeinfo never
  # runs and the absent prerequisite stops mattering.
  #
  # We ship a toolchain, not a manual. Skipping docs also removes the coupling
  # rather than satisfying it: the build stops caring whether the runner image
  # carries texinfo, or which version of it.
  make MAKEINFO=true || { echo "retrying serially (libgcc race)"; make -j1 MAKEINFO=true; }
  make install MAKEINFO=true
  touch .done
fi
